...
fs.readFile('gimnasio.csv', 'utf-8', (err, data) => {
  ...
  for (const element of array) {
    ...
    for (let i = 0; i < arrayValues.length; i++) {
      if (keys[i] === 'PASSWORD') {
        const encriptado = crypto.AES.encrypt(
          arrayValues[i],
          '98f3hfsdfuisdhf98'
        )
  ...
  const passwordDesencrypted = crypto.AES.decrypt(
    ARRAYUSERS[0].PASSWORD,
    '98f3hfsdfuisdhf98'
  )
  fs.writeFile(
    'primerUser.txt',
    `${ARRAYUSERS[0].NIF}: ${passwordDesencrypted.toString(crypto.enc.Utf8)}`,
    () => {
      console.log('escrito')
    }
  )
